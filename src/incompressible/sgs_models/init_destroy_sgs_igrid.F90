...
subroutine init(this, gpC, gpE, spectC, spectE, dx, dy, dz, inputfile, zMeshE, zMeshC, fBody_x, fBody_y, fBody_z, computeFbody, PadeDer, cbuffyC, cbuffzC, cbuffyE, cbuffzE, rbuffxC, rbuffyC, rbuffzC, rbuffyE, rbuffzE, Tsurf, ThetaRef, wTh_surf, Fr, Re, isInviscid, isStratified, botBC_temp, initSpinUp)
  ...
  real(rkind), dimension(:,:), allocatable :: z0_surf    ! YIS (flexible size)
  ...
  ! YIS start
  allocate(z0_surf(gpC%zsz(1),gpC%zsz(2)))
  if (z0_field) then
  ...
  this%mid = SGSmodelID
  ! YIS vary z0 field start
  this%z0_field = z0_field
  allocate(this%z0_surf(gpC%zsz(1),gpC%zsz(2)))
  this%z0_surf = z0_surf
  ! YIS vary z0 field end 
  this%z0  = z0
  this%z0t = z0t
